...
For this part, we seek to eliminate genes with low expression (cutoff: -5) and/or low variability (cutoff: 0.8).

```{r }
# Filtering genes with low expression (-dCT < -5)
expr_filtered <- norm_data_pca[,-1] %>%
  select(where(~ mean(.x, na.rm = TRUE) > -5))
```
...
Herein, we require 10 components to achieve over 75% of the explained variance.
...
for_heat <- norm_data_pca %>%
  ...
  select(Name, Localization, all_of(top_genes))
